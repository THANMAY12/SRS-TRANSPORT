...
import { Building2, Search, Check, Printer } from "lucide-react";
...
export const PendingAdvanceCompanyPage = ({
  ...
  const [advRecType, setAdvRecType] = useState("Cash");
  ...
  const handleOpenModal = (trip) => {
    ...
    setAdvRecAmount(String(trip.advanceReceivedAmount || 0));
    setAdvRecType("Cash");
  };

  const handleSaveAdvanceReceived = async (e) => {
    e.preventDefault();
    if (!selectedTrip) return;
    if (!advRecType) {
      alert("Please select Advance Received Type (Cash, PhonePe, or To Pay).");
      return;
    }
    ...
    try {
      await onUpdateTrip(selectedTrip.id, {
        advanceReceivedAmount: Number(advRecAmount) || 0,
        advanceReceivedType: advRecType,
      });
      setSelectedTrip(null);
    } catch (err) {
      alert(err.message || "Failed to update company advance");
    } finally {
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">
            Pending Advance Company ({pendingTrips.length})
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Automatically displays records where Advance Received Type is blank.
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-72">
            ...
            <input
              type="text"
              placeholder="Search pending company advance..."
              value={globalSearch}
              onChange={(e) => setGlobalSearch(e.target.value)}
              className="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
            />
          </div>
          <button
            id="pending-advance-company-print-btn"
            ...
            title="Print Pending Company Advance Table"
          >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-cyan-500/10 text-slate-700 dark:text-slate-200 font-bold uppercase tracking-wider border-b border-cyan-500/20">
              <tr>
                ...
                <th className="py-3.5 px-4">Advance Received Amount</th>
                <th className="py-3.5 px-4">Collection Due Date</th>
              ...
              {pendingTrips.length === 0 ? (
                <tr>
                  <td colSpan={12} className="py-12 text-center text-slate-400">
                    🎉 No pending company advances! All company advance
                    collection types are recorded.
                  </td>
                </tr>
              ) : (
                pendingTrips.map((trip) => {
                  const daysPending = calculateDaysPending(
                    trip.collectionDueDate || trip.date,
                  );
                  return (
                    <tr
                      key={trip.id}
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-xs shadow-xs flex items-center justify-center gap-1 mx-auto transition-transform active:scale-95"
                        >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <h3 className="text-base font-bold text-slate-900 dark:text-white mb-1">
              Select Advance Received Type for Trip #{selectedTrip.slNo}
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
              ...
              | Transport: {selectedTrip.transport}
            </p>

            <form
              onSubmit={handleSaveAdvanceReceived}
            ...
            >
              <div>
                ...
                <input
                  ...
                  min="0"
                  value={advRecAmount}
                  onChange={(e) => setAdvRecAmount(e.target.value)}
                  className="w-full px-3 py-2 text-sm font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500"
                />
              </div>
              ...
              <div>
                ...
                <select
                  required
                  value={advRecType}
                  onChange={(e) => setAdvRecType(e.target.value)}
                  className="w-full px-3 py-2 text-xs font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500"
                >
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold shadow-md shadow-cyan-600/20 flex items-center gap-1.5"
                >
