      ...
      <div className="space-y-5 text-xs">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 p-3.5 rounded-lg bg-slate-50 border border-slate-200">
            <div>
              ...
              <strong className="font-mono text-slate-900">{formatCurrency(trip.advancePaidAmount)}</strong>
              <span className="ml-2 text-[10px] font-medium text-slate-600">
                ({trip.advancePaidType || "Blank / Pending"})
              </span>
            </div>
            <div>
              <span className="text-slate-500 block text-[10px]">Advance Received from Company</span>
              <strong className="font-mono text-slate-900">{formatCurrency(trip.advanceReceivedAmount)}</strong>
              <span className="ml-2 text-[10px] font-medium text-slate-600">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 p-3.5 rounded-lg bg-slate-50 border border-slate-200 font-mono">
            <div>
              <span className="text-slate-500 block text-[10px] font-sans">Vehicle Balance Due</span>
            ...
            <div>
              <span className="text-slate-500 block text-[10px] font-sans">Company Balance Due</span>
              <strong className="text-slate-900">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-200">
          <div>
            ...
            {isComp ? (
              ...
              <StatusBadge type="pending-advance-vehicle" text="Pending vehicle advance" size="sm" />
            ) : isAdvCompPending ? (
              <StatusBadge type="pending-advance-company" text="Pending company advance" size="sm" />
            ) : (
